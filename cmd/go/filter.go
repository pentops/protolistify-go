...
func genFilterStatementMethods(gen *protogen.Plugin, g *protogen.GeneratedFile, receiver string) {
	g.P("func (r *", receiver, ") FilterStatements() ([]string, []interface{}){")
	g.P("stmts, args := r.FilterStatementsWithPlaceholders()")
	g.P()
	g.P("ordinal := 1")
	g.P("for i, stmt := range stmts {")
	g.P("if ", stringsPackage.Ident("Index"), `(stmt, "?") == -1 {`)
	g.P("continue")
	g.P("}")
	g.P()
	g.P("buf := &", bytesPackage.Ident("Buffer"), "{}")
	g.P("for {")
	g.P("p := ", stringsPackage.Ident("Index"), `(stmt, "?")`)
	g.P("if p == -1 {")
	g.P("break")
	g.P("}")
	g.P()
	g.P(`if len(stmt[p:]) > 1 && stmt[p:p+2] == "??" { // escape ?? => ?`)
	g.P("buf.WriteString(stmt[:p])")
	g.P(`buf.WriteString("?")`)
	g.P("if len(stmt[p:]) == 1 {")
	g.P("break")
	g.P("}")
	...
	g.P("} else {")
	g.P("buf.WriteString(stmt[:p])")
	g.P(`buf.WriteString(fmt.Sprintf("$%d", ordinal))`)
	g.P("stmt = stmt[p+1:]")
	g.P("ordinal++")
	g.P("}")
	g.P("}")
	g.P()
	g.P("stmts[i] = buf.String()")
	g.P("}")
	g.P()
	...
	genFilterStatmentMethodsCase(g, "Date", "Min.Value", "Max.Value")
	genFilterStatmentMethodsCase(g, "Timestamp", "Min.AsTime()", "Max.AsTime()")
	g.P("}")

	g.P("} else {")
	g.P(`statements = append(statements, `, fmtPackage.Ident("Sprintf"), `("%s = ?", filter.Field))`)
	g.P("args = append(args, filter.GetValue())")
	g.P("}")
	g.P("}")
	g.P()
	g.P("return statements, args")
	g.P("}")
	g.P()
}
...
func genFilterStatmentMethodsCase(g *protogen.GeneratedFile, name, minComp, maxComp string) {
	...
	g.P(fmt.Sprintf("if r.%s.Min != nil && r.%s.Max != nil {", name, name))
	g.P("statements = append(statements, ", fmtPackage.Ident("Sprintf"), "(`%s >= ? AND %s <= ?`, filter.Field, filter.Field))")
	g.P(fmt.Sprintf("args = append(args, r.%s.%s, r.%s.%s)", name, minComp, name, maxComp))
	g.P("} else {")
	g.P(fmt.Sprintf("if r.%s.Min != nil {", name))
	g.P("statements = append(statements, ", fmtPackage.Ident("Sprintf"), "(`%s >= ?`, filter.Field))")
	g.P(fmt.Sprintf("args = append(args, r.%s.%s)", name, minComp))
	g.P("}")
	g.P(fmt.Sprintf("if r.%s.Max != nil {", name))
	g.P("statements = append(statements, ", fmtPackage.Ident("Sprintf"), "(`%s <= ?`, filter.Field))")
	g.P(fmt.Sprintf("args = append(args, r.%s.%s)", name, maxComp))
	g.P("}")
	g.P("}")
}
